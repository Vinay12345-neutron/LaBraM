...
- Trains ONLY top 2 Transformer blocks (blocks.10 and blocks.11), fc_norm, and head (966,361 params)
...
OUT_ROOT = WORKDIR / "runs" / "ablation_top2_loso"
LOG_ROOT = WORKDIR / "log" / "ablation_top2_loso"
...
def main():
    parser = argparse.ArgumentParser(description="Run Ablation 3 (Top-2 Blocks Fine-Tuning) under 73-Fold LOSO CV")
    ...
    print("=" * 95)
    print("STARTING ABLATION 3: TOP-2 TRANSFORMER BLOCKS (11-12) + HEAD UNDER 73-FOLD LOSO CV")
    print("=" * 95)
    print(f"Model Architecture       : {MODEL} (~5.8M params)")
    print(f"Pretraining Checkpoint   : {CKPT_PATH}")
    print(f"Trainable Parameters     : 966,361 (Blocks 11-12 + fc_norm + head, ~16.66%)")
    print(f"Frozen Parameters        : 4,832,776 (Blocks 1-10 + patch embeddings, ~83.34%)")
    print(f"Evaluation Protocol      : Leave-One-Subject-Out (LOSO) across 73 Subjects")
    print(f"Epochs per Fold          : {args.epochs}")
    print(f"Batch Size               : {BATCH}")
    print(f"Learning Rate            : {LR} (AdamW, Cosine Annealing, Layer Decay 0.9)")
    ...
    for fold_idx, split_path in enumerate(split_files):
        ...
        if args.dry_run:
            ...
            if fold_idx == 0:
                print("\n  Executing 1-fold dry run for Fold 1 (S1)...")
    ...
    print("\n" + "=" * 95)
    print("ALL 73 LOSO FOLDS COMPLETED FOR ABLATION 3 (TOP-2 FINE-TUNING)!")
    print(f"Checkpoints and outputs saved in: {OUT_ROOT}")
    print("Run `python evaluate_ablation_top2_loso.py` to extract full per-subject and pooled metrics.")
    print("=" * 95)
